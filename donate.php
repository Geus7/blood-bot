...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['submit_csv']) && isset($_FILES['csv_file']) && $_FILES['csv_file']['error'] === 0) {
        ...
        while (($row = fgetcsv($csvFile)) !== false) {
            ...
            $quantity = (float)$row[2];  // Treat quantity as float (e.g., 6.00 liters)
            $request_date = htmlspecialchars($row[3]);

            // Insert into the donations table instead of requests (assuming it's the correct table)
            $stmt = $pdo->prepare("INSERT INTO donations (blood_type, location, quantity, donation_date) VALUES (?, ?, ?, ?)");
        ...
        $_SESSION['csv_success'] = "CSV upload successful! $inserted donations added.";
        if ($errors > 0) {
            $_SESSION['csv_error'] = "$errors rows failed to insert.";
        }
    }

    // Process manual form input if 'Submit Donation' button was clicked
    elseif (isset($_POST['submit_manual'])) {
        // Capture form data
        $blood_type = $_POST['blood_type'];
        $location = $_POST['location'];
        $quantity = (float)$_POST['quantity']; // Treat quantity as float
        $donation_date = $_POST['request_date']; // Changed to donation_date

        // Insert into donations table instead of requests (assuming it's the correct table)
        $stmt = $pdo->prepare("INSERT INTO donations (blood_type, location, quantity, donation_date) VALUES (?, ?, ?, ?)");
        if ($stmt->execute([$blood_type, $location, $quantity, $donation_date])) {
            // Set success message for manual form submission
            $_SESSION['manual_success'] = "Blood donation submitted successfully.";
        } else {
            // Set error message for manual form submission
            $_SESSION['manual_error'] = "Error submitting donation.";
        }
    }
...
<body>
    <div class="container">
        ...
        <form method="post" enctype="multipart/form-data">
            <h2>Submit a Blood Donation Request Manually</h2>
            ...
            <label for="request_date">Donation Date:</label>
            <input type="date" id="request_date" name="request_date" required>
            
            <button type="submit" name="submit_manual">Submit Donation</button>
        </form>

        <!-- CSV Upload Form -->
        <form method="post" enctype="multipart/form-data">
            <h2>Or Upload a CSV File for Donations</h2>
